...
class App(tk.Tk):
    ...
    def show_smart_management(self, don_hang=None):
        self.clear_frame()

        if don_hang:
            ttk.Label(self.main_frame, text=f"Đơn hàng: {don_hang.ten_don_hang}", font=("Arial", 12, "bold")).grid(row=0, column=0, sticky=tk.W)
            ttk.Label(self.main_frame, text=f"Khách hàng: {don_hang.ten_khach_hang}", font=("Arial", 12, "bold")).grid(row=1, column=0, sticky=tk.W)
            ttk.Label(self.main_frame, text=f"Thời gian yêu cầu hoàn thành: {don_hang.thoi_gian_yeu_cau_hoan_thanh}", font=("Arial", 12, "bold")).grid(row=2, column=0, sticky=tk.W)

            # Hiển thị thông tin sản phẩm thuộc đơn hàng
            ttk.Label(self.main_frame, text="Sản phẩm:", font=("Arial", 12, "bold")).grid(row=3, column=0, sticky=tk.W)
            for i, sp in enumerate(don_hang.don_hang_san_pham, start=1):
                ttk.Label(self.main_frame, text=f"{sp.san_pham.ten_san_pham} (x{sp.so_luong})", font=("Arial", 12)).grid(row=3+i, column=0, sticky=tk.W)

        # Khung quản lý nhân công
        frame = ttk.Frame(self.main_frame, padding="10")
        frame.grid(row=4 + len(don_hang.don_hang_san_pham), column=0, sticky=(tk.W, tk.E))

        # Nhãn và biến số nhân công hiện tại
        ...
        SoNhanCong = 0
        for nhan_cong in nhan_cong_data:
            if nhan_cong.id == last_nhan_cong_date:
                SoNhanCong = nhan_cong.tong_nhan_cong
                
        self.current_workers = tk.IntVar(value=SoNhanCong)
        ttk.Label(frame, text="Số nhân công hiện tại:").grid(row=0, column=0, sticky=tk.W)
        ttk.Label(frame, textvariable=self.current_workers).grid(row=0, column=1, sticky=tk.W)

        # Nút thêm và bớt nhân công
        ttk.Button(frame, text="Thêm nhân công", command=self.add_worker).grid(row=1, column=0, sticky=tk.W)
        ttk.Button(frame, text="Bớt nhân công", command=self.remove_worker).grid(row=1, column=1, sticky=tk.W)

        # Nhãn tổng số nhân công
        self.total_workers_label = ttk.Label(frame, text="Tổng số nhân công hiện tại: 0")
        self.total_workers_label.grid(row=2, column=0, columnspan=2, sticky=tk.W)

        # Nhãn thời gian hoàn thành
        self.completion_time_label = ttk.Label(frame, text="Thời gian hoàn thành: 0 phút")
        self.completion_time_label.grid(row=3, column=0, columnspan=2, sticky=tk.W)

        # Khung cho các giai đoạn
        stages_frame = ttk.Frame(self.main_frame, padding="10")
        stages_frame.grid(row=5 + len(don_hang.don_hang_san_pham), column=0, sticky=(tk.W, tk.E))

        giai_doan_data = self.giai_doan_controller.get_giai_doan()

        # Lọc các giai đoạn liên quan đến sản phẩm trong đơn hàng
        related_giai_doan_data = [gd for gd in giai_doan_data if any(sp.san_pham.id == gd.id_quy_trinh_san_xuat for sp in don_hang.don_hang_san_pham)]

        # Tạo các slider cho các giai đoạn liên quan
        self.stage_vars = []
        self.giai_doan_data = related_giai_doan_data  # Lưu dữ liệu giai đoạn để sử dụng trong tính toán
        for i, giai_doan in enumerate(related_giai_doan_data):
            stage_var = tk.IntVar(value=0)
            self.stage_vars.append(stage_var)
            ttk.Label(stages_frame, text=f"Quy trình {giai_doan.id_quy_trinh_san_xuat}, giai đoạn {giai_doan.thu_tu}").grid(row=i, column=0, sticky=tk.W)
            slider = ttk.Scale(stages_frame, from_=0, to=giai_doan.so_nhan_cong_nang_suat_lon_nhat, orient=tk.HORIZONTAL, variable=stage_var, command=lambda e, idx=i: self.update_total_workers_and_completion_time(idx, don_hang))
            slider.grid(row=i, column=1, sticky=(tk.W, tk.E))

    def update_total_workers_and_completion_time(self, idx=None, don_hang=None):
        total_workers = sum(stage_var.get() for stage_var in self.stage_vars)
        self.total_workers_label.config(text=f"Tổng số nhân công hiện tại: {total_workers}")

        # Tính toán thời gian hoàn thành dựa trên giá trị của các thanh trượt
        total_time = 0
        time_per_quy_trinh = {}

        for i, stage_var in enumerate(self.stage_vars):
            num_workers = stage_var.get()
            if num_workers > 0:
                giai_doan = self.giai_doan_data[i]
                related_san_pham = next(sp for sp in don_hang.don_hang_san_pham if sp.san_pham.id == giai_doan.id_quy_trinh_san_xuat)
                processing_time_per_item = giai_doan.thoi_gian_xu_ly_toi_thieu * max(1, giai_doan.so_nhan_cong_nang_suat_lon_nhat / num_workers)
                total_time_for_stage = processing_time_per_item * related_san_pham.so_luong

                if giai_doan.id_quy_trinh_san_xuat not in time_per_quy_trinh:
                    time_per_quy_trinh[giai_doan.id_quy_trinh_san_xuat] = 0

                time_per_quy_trinh[giai_doan.id_quy_trinh_san_xuat] += total_time_for_stage

        # Lấy thời gian lớn nhất giữa các quy trình
        total_time = max(time_per_quy_trinh.values(), default=0)
        self.completion_time_label.config(text=f"Thời gian hoàn thành: {total_time:.2f} phút")


    def add_worker(self):
        # Hiển thị hộp thoại để nhập số lượng nhân công cần thêm
        num_to_add = simpledialog.askinteger("Thêm nhân công", "Nhập số lượng nhân công cần thêm:", minvalue=1)
        if num_to_add is not None:
            self.current_workers.set(self.current_workers.get() + num_to_add)
            self.update_total_workers_and_completion_time()

    def remove_worker(self):
        # Hiển thị hộp thoại để nhập số lượng nhân công cần bớt
        num_to_remove = simpledialog.askinteger("Bớt nhân công", "Nhập số lượng nhân công cần bớt:", minvalue=1)
        if num_to_remove is not None and self.current_workers.get() >= num_to_remove:
            self.current_workers.set(self.current_workers.get() - num_to_remove)
            self.update_total_workers_and_completion_time()
        elif num_to_remove is not None:
            messagebox.showwarning("Cảnh báo", "Số lượng nhân công cần bớt lớn hơn số nhân công hiện tại")
    ...
    def show_bang_chuyen_list(self):
        # Lưu trữ trạng thái hiện tại của các băng chuyền
        bang_chuyen_state = {}
        if self.bang_chuyen_data:
            for bc in self.bang_chuyen_data:
                try:
                    bang_chuyen_state[bc.id] = {
                        'running': bc.running,
                        'text_content': bc.text_content if bc.text_content else "",
        ...
        for quy_trinh, bang_chuyens in quy_trinh_dict.items():
            ...
            for row, bang_chuyen in enumerate(bang_chuyens, start=1):
                ...
                if bang_chuyen.id in bang_chuyen_state:
                    bang_chuyen.text_widget.insert(tk.END, bang_chuyen_state[bang_chuyen.id]['text_content'])
                ...
                if bang_chuyen.id in bang_chuyen_state and bang_chuyen_state[bang_chuyen.id]['running']:
                    bang_chuyen_state[bang_chuyen.id]["thread"] = None
                    bang_chuyen_state[bang_chuyen.id]["running"] = False
    ...
    def simulate_bang_chuyen(self, bang_chuyen, text_widget, tai_hien_tai_label):
        ...
        while bang_chuyen.running:
            ...
            if bang_chuyen.tai_hien_tai < bang_chuyen.suc_chua:
                ...
                message = f"Thêm 1 sản phẩm lên băng chuyền {bang_chuyen.ten_bang_chuyen} (Tổng: {bang_chuyen.tai_hien_tai})"
            else:
                self.controller.dichuyen_san_pham(san_pham_id, bang_chuyen.id)
                bang_chuyen.tai_hien_tai -= 1
                message = f"Di chuyển 1 sản phẩm khỏi băng chuyền {bang_chuyen.ten_bang_chuyen}"
            ...
            if bang_chuyen.text_widget and bang_chuyen.text_widget.winfo_exists():
                ...
                if self.current_menu is None or self.current_menu == self.show_bang_chuyen_list:
    ...
    def show_don_hang_list(self):
        ...
        for row, don_hang in enumerate(don_hang_data, start=1):
            ...
            tinhtoan_button = tk.Button(control_frame, text="Tính toán", command=lambda dh=don_hang: self.show_smart_management(dh))
            tinhtoan_button.pack(side=tk.LEFT, padx=5, pady=2)
